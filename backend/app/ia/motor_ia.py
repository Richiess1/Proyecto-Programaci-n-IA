...
from app.ia.completitud import asegurar_completitud
...
def evaluar_idea(idea: dict) -> ResultadoMotorIA:
    ...
    Primero verifica la completitud de la entrada: si es insuficiente, corta con
    EntradaIncompletaError SIN llamar al proveedor (cero tokens). Si pasa, arma el
    prompt (system estático + datos variables), llama al proveedor de IA vía el
    cliente, y devuelve la evaluación validada junto con la traza (prompt,
